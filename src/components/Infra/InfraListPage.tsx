...
const InfraRow: React.FC<RowProps<K8sResourceCommon>> = ({ obj, index, style }) => (
  <TableRow id={obj.metadata.uid} index={index} trKey={obj.metadata.uid} style={style}>
    <TableData>
      <Link to={`/k8s/ns/${obj.metadata.namespace}/agent-install.openshift.io~v1beta1~InfraEnv/${obj.metadata.name}`}>
        {obj.metadata.name}
      </Link>
    </TableData>
  </TableRow>
  ...
  return (
    <>
      <ListPageHeader title="CIM Infras">
        <ListPageCreate groupVersionKind="agent-install.openshift.io~v1beta1~InfraEnv">
          Create
        </ListPageCreate>
      </ListPageHeader>
      <ListPageBody>
